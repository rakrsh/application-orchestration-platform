...
def start_asset_services() -> List[int]:
    ...
    for name, config in manifest.items():
        ...
        cwd = (
            REPO_ROOT / config.get("cwd", "assets")
            if not Path(config.get("cwd", "assets")).is_absolute()
            else Path(config.get("cwd", "assets"))
        )
...
def cmd_start(args: argparse.Namespace) -> None:
    if STATE_FILE.exists():
        logger.warning(
            "Platform is already running (state file exists). Use 'platform stop' first."
        )
        sys.exit(1)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Application Orchestration Platform CLI"
    )
    ...
    subparsers.add_parser(
        "uninstall", help="Stop services and remove installed dependencies"
    )
